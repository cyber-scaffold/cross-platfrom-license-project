...
export async function getResourcePathInfo(): Promise<ResourcePathInfoType> {
  ...
  const assetsDirectoryPath = slash(path.resolve(projectDirectoryPath, assetsDirectoryName));
  /**
   * 公共资源所在的目录比如要向前端浏览器提供的dll动态链接库文件
   * 框架层的基准目录是根据 项目根目录的绝对路径 计算得到的
   * **/
  const extractResourceDirectory = slash(path.join(assetsDirectoryPath, "extract"));
  /**
   * 公共资源所在的目录比如要向前端浏览器提供的dll动态链接库文件
   * 框架层的基准目录是根据 项目根目录的绝对路径 计算得到的
   * **/
  const publicResourceDirectory = slash(path.join(assetsDirectoryPath, "public"));
  /**
   * 项目中的静态资源指向的目录
   * 框架层的基准目录是根据 项目根目录的绝对路径 计算得到的
   * **/
  const projectStaticResourceDirectory = slash(path.join(assetsDirectoryPath, "statics"));
  /**
   * Swagger静态资源所在的目录
   * **/
  const swaggerResourceDirectory = slash(path.join(assetsDirectoryPath, "swagger"));
  return {
    assetsDirectoryName,
    assetsDirectoryPath,
